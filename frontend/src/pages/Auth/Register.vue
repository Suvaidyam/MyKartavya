  ...
  <div class="flex pt-[62px] flex-col items-center min-h-screen bg-secondary" style="background: #F5F5F5;">
    ...
    <main class="flex flex-col items-center text-center w-full px-4 min-h-[calc(100vh-64px)] justify-center">
      ...
      <div class="mt-8 bg-white p-6 rounded-sm w-full max-w-[443px] shadow-md">
        ...
        <p class="text-sm mt-4 text-gray-600">Already have an account?
...
const get_otp = async () => {
  ...
  try {
    loading.value = true;
    const response = await call('mykartavya.controllers.email.register_send_otp', { email: email.value });
    if (response.status === "success") {
      loading.value = false;
      toast.success('OTP sent successfully');
      router.push({ name: 'Verify', query: { email: email.value, full_name: full_name.value } });
    } else {
      loading.value = false;
