...
def get_columns() -> list[dict]:
	...
	return [
		...
		},
		{
			...
			"fieldname": "total_karma_points",
			"fieldtype": "Int",
			"width": 130,
		},
		{
			"label": _("Time Donated"),
			"fieldname": "time_donated",
			"fieldtype": "Data",
			"width": 120,
		},
		{
...
def get_data(filters: dict | None = None):
	...
	if filters:
		...
		if filters.get("from_date"):
			conditions.append(f"DATE(su.creation) >= '{filters.get('from_date')}'")
		if filters.get("to_date"):
			conditions.append(f"DATE(su.creation) <= '{filters.get('to_date')}'")
